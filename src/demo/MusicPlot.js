...
import ParallelAxes from './Paralle'
...
const convertTime = (data) =>{
  ...
  for(var i=1; i < data.length; i++) {
    var time = data[i].EndTime;
    var arr = time.split(':');
    let res = 0;
    if(arr.length ==3){
      res = Number(arr[0]) * 60 * 60 * 1000 + Number(arr[1]) * 60 * 1000 + Number(arr[2])*1000;
    }
    if(arr.length ==2){
      res = Number(arr[0]) * 60 * 60 * 1000 + Number(arr[1]) * 60 * 1000;
    }
    console.log("res.........",res);
    data[i].EndTime = res;
  }
...
const renderLines = (mid, props, scale, yLocs, zoom, bundle, participantsMap ) => {
  return (coords, index) => {
    console.log("coords..............................",coords);
    console.log("index......", index);

    var p1 = + participantsMap.get(coords.Attacker)-1;
    var p2 = + participantsMap.get(coords.Victim)-1;
    ...
    const lineProps = {
      ...
      fill: 'none',
      stroke: d3Col.schemeSet2[+ participantsMap.get(coords.Attacker) - 1],
      opacity: 1,
      key: index,
      //    x1: scale(index),
      ...
      markerEnd: "url(#arrow"+ participantsMap.get(coords.Attacker) +")",
    }
    ...
    var points = [
      {x: scale(index), y: yLocs[+ participantsMap.get(coords.Attacker)-1]},
      {x: scale(index), y: yLocs[+ participantsMap.get(coords.Victim)-1]}
    ];
...
export default class MusicNotation extends React.Component{
  ...
  componentWillMount() {
    this.setState({scaleTime: null, scaleEq: null, yLocs:[], axLabels:[], top:0 , left: 0, anno: null,
      txtValue: "", childVisible: false, mid: [], bundle: true,
      data:[],participants: new Map(),
      participantsName:[]});
  }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(nextProps.maxLen != this.props.maxLen){
      console.log("nextProps............",nextProps.maxLen);
  ...
  applyFilter() {
    if(this.props.dataCont != null && this.props.dataCont.length >0) {
      console.log("dataCont............",this.props.dataCont);
      ...
      if(this.props.participants != null){
        ...
        this.state.participants = participantsMap;
      }

      var scalingprops = { ...this.props, width: this.props.width*this.props.zoom};
      //this.state.scaleTime = xScaleTime(scalingprops);
      this.state.scaleEq = xScaleEq(scalingprops, this.props.maxLen);
      this.state.axLabels = ["1","2","3","4", "5"];

      this.state.yLocs = [];
      var axDist = (this.props.height - this.props.padding*3)/(6-1);
      ...
      if(this.state.bundle){
        ...
        var stop = 0;
        for(var i=1; i < data.length; i++) {
          if(participantsMap.get([data[i].Attacker]) === participantsMap.get(data[i-1].Attacker)
            && participantsMap.get(data[i].Victim) === participantsMap.get(data[i-1].Victim)) {
            count = count + 1;
          }
          else {
            stop = i - 1;
            var minP = d3.min([this.state.yLocs[+ participantsMap.get(data[i-1].Attacker)-1],
              this.state.yLocs[+participantsMap.get(data[i-1].Victim)-1]]);
            var maxP = d3.max([this.state.yLocs[+participantsMap.get(data[i-1].Attacker)-1],
              this.state.yLocs[+participantsMap.get(data[i-1].Victim)-1]]);
        ...
        stop = data.length - 1;
        var minP = d3.min([this.state.yLocs[+participantsMap.get(data[stop].Attacker)-1],
          this.state.yLocs[+participantsMap.get(data[stop].Victim)-1]]);
        var maxP = d3.max([this.state.yLocs[+participantsMap.get(data[stop].Attacker)-1],
          this.state.yLocs[+participantsMap.get(data[stop].Victim)-1]]);
        var y = minP + Math.abs(maxP - minP)/2;
  ...
  render() {
    ...
    return (
      <div>
        ...
        {/*</div>*/}
        <div style={{width:this.props.width, overflowX: "auto", overflowY: "hidden",
          background: "#ffffff", marginRight:20, marginTop:20}}>
          <svg width={this.props.width*this.props.zoom} height={this.props.height} padding={5}>
            ...
            <marker ref="marker5" id="arrow5">
              <path d="M 0 0 L 6 2 L 0 4 z" fill={d3Col.schemeSet2[4]} />
            </marker>
            <marker ref="marker6" id="arrow6">
              <path d="M 0 0 L 6 2 L 0 4 z" fill={d3Col.schemeSet2[5]} />
            </marker>
            ...
            { this.props.dataCont === null
              ...
              : <g>
                <g className="scatterArea">
                  {this.props.dataCont.map(renderLines(this.state.mid, this.props, scale, this.state.yLocs,
                    this.props.zoom, this.state.bundle, this.state.participants))}
                </g>
